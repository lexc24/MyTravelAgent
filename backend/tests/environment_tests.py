...
class EnvironmentVariableTests(TestCase):
    ...
    def test_debug_setting_for_environment(self):
        ...
        if env in ["production", "staging"]:
            self.assertFalse(
                settings.DEBUG, f"DEBUG must be False in {env} environment"
            )

        # Additional check using Render's environment variable
        if os.environ.get("RENDER"):
            self.assertFalse(
                settings.DEBUG, "DEBUG must be False when deployed on Render"
            )
    ...
    def test_gemini_api_configuration(self):
        ...
        if not settings.DEBUG:
            self.assertIsNotNone(
                gemini_key, "Gemini API key must be set for recommendation engine"
            )

            # Basic validation of key format
            if gemini_key:
                self.assertGreater(
                    len(gemini_key), 20, "Gemini API key seems too short"
                )
...
class DatabaseConfigurationTests(TransactionTestCase):
    ...
    def test_database_migrations_applied(self):
        ...
        self.assertEqual(
            len(unapplied), 0, f"Found unapplied migrations: {', '.join(unapplied)}"
        )
    ...
    def test_database_tables_exist(self):
        ...
        with connection.cursor() as cursor:
            ...
            self.assertEqual(
                len(missing_tables), 0, f"Missing required tables: {missing_tables}"
            )
...
class RenderDeploymentTests(TestCase):
    ...
    def test_render_environment_variables(self):
        ...
        for var, description in essential_render_vars.items():
            ...
            self.assertIsNotNone(
                value, f"Render environment variable {var} not found: {description}"
            )
    ...
    def test_static_files_configuration(self):
        ...
        whitenoise_index = middleware_list.index(
            "whitenoise.middleware.WhiteNoiseMiddleware"
        )
...
class LoggingConfigurationTests(TestCase):
    ...
    def test_logging_configured(self):
        ...
        self.assertTrue(
            hasattr(settings, "LOGGING"), "LOGGING configuration should be defined"
        )
