...
class CompleteUserJourneyTests(APITestCase):
    ...
    def test_full_user_registration_to_trip_planning(self):
        ...
        token_url = reverse("get_token")
        login_data = {"username": "traveler", "password": "SecurePass123!"}
        ...
        pref_response = self.client.patch(
            preferences_url, preferences_data, format="json"
        )
        ...
        with patch("destination_search.views.workflow_manager") as mock_wf:
            ...
            mock_wf.get_next_question.return_value = (
                "What is your preferred climate - tropical or Mediterranean?"
            )
        ...
        session_data = {"trip": trip_id, "current_stage": "destination"}
    ...
    def test_trip_planning_with_destination_selection(self):
        ...
        trip = Trip.objects.create(
            user=user, title="Destination Selection Test", status="planning"
        )
...
class APIIntegrationTests(APITestCase):
    ...
    def test_trip_lifecycle_api_flow(self):
        ...
        create_response = self.client.post(
            "/api/trips", {"title": "API Test Trip", "budget": "3000"}
        )
        ...
        update_response = self.client.patch(
            f"/api/trips/{trip_id}",
            {"description": "Updated description", "travelers_count": 3},
        )
...
def test_planning_session_workflow(self):
    ...
    # Create trip
    trip = Trip.objects.create(
        user=self.user, title="Planning Workflow Test", status="planning"
    )

    # Create planning session starting at 'destination'
    session_response = self.client.post(
        "/api/planning-sessions", {"trip": trip.id, "current_stage": "destination"}
    )
    self.assertEqual(session_response.status_code, status.HTTP_201_CREATED)
    ...
    for current_stage, next_stage in expected_progression:
        advance_response = self.client.post(
            f"/api/planning-sessions/{session_id}/advance_stage"
        )
...
class DestinationSearchIntegrationTests(APITestCase):
    ...
    def setUp(self):
        ...
        self.trip = Trip.objects.create(
            user=self.user, title="Search Test Trip", status="planning"
        )
    ...
    def test_complete_recommendation_workflow(self, mock_wf):
        ...
        mock_wf.get_next_question.return_value = (
            "What is your budget for this adventure trip?"
        )
        ...
        response = self.client.post(
            "/destination_search/chat",
            {"trip_id": self.trip.id, "message": "My budget is around $3000"},
        )
        ...
        response = self.client.post(
            "/destination_search/chat",
            {"trip_id": self.trip.id, "message": "10 days with moderate fitness level"},
        )
    ...
    def test_conversation_reset(self):
        ...
        ConversationState.objects.create(
            conversation=conversation, current_stage="asking_clarifications"
        )

        # Set trip destination
        destination = Destination.objects.create(
            name="Test Dest", country="Test Country"
        )
...
class ExternalServiceIntegrationTests(TransactionTestCase):
    ...
    def test_database_transaction_rollback(self):
        ...
        try:
            with transaction.atomic():
                # Create a trip
                trip = Trip.objects.create(user=self.user, title="Transaction Test")
    ...
    def test_concurrent_user_sessions(self):
        ...
        self.assertEqual(
            successful, 5, "All concurrent users should successfully create trips"
        )
...
class ErrorHandlingIntegrationTests(APITestCase):
    ...
    def test_api_error_responses(self):
        ...
        response = self.client.post(
            "/api/token", {"username": "tester", "password": "wrongpassword"}
        )
        self.assertEqual(response.status_code, status.HTTP_401_UNAUTHORIZED)
        ...
        response = self.client.post(
            "/api/trips",
            {"title": "", "budget": "not-a-number"},  # Empty title  # Invalid budget
        )
    ...
    def test_destination_search_error_handling(self):
        ...
        # Invalid trip ID
        response = self.client.post(
            "/destination_search/chat", {"trip_id": 99999, "message": "Hello"}
        )
        self.assertEqual(response.status_code, status.HTTP_404_NOT_FOUND)

        # Missing required fields
        response = self.client.post(
            "/destination_search/chat", {"message": "Hello"}  # Missing trip_id
        )
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
        ...
        response = self.client.post(
            "/destination_search/chat",
            {"trip_id": trip.id, "message": "   "},  # Whitespace only
        )
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
    ...
    def test_graceful_ai_failure_handling(self, mock_wf):
        ...
        mock_wf.process_initial_message.side_effect = Exception(
            "AI service unavailable"
        )

        response = self.client.post(
            "/destination_search/chat",
            {"trip_id": trip.id, "message": "I want a vacation"},
        )
        ...
        self.assertNotIn(
            "AI service unavailable", str(response.data)
