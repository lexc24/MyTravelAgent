...
class DatabasePerformanceTests(TransactionTestCase):
    ...
    def test_database_indexes_exist(self):
        ...
        with connection.cursor() as cursor:
            ...
            if 'postgresql' in connection.vendor:
                ...
                """)
                result = cursor.fetchone()[0]
                self.assertGreater(result, 0, "Missing index on Message.conversation_id")
    ...
    def test_n_plus_one_queries(self):
        ...
        with self.assertNumQueries(3):  # Should use select_related/prefetch_related
            ...
            # Force evaluation
            trip_data = [
                {'title': t.title, 'dest': t.destination.name if t.destination else None}
                for t in trips
            ]
    ...
    def test_bulk_operations_performance(self):
        ...
        trips_to_create = [
            Trip(user=self.user, title=f'Bulk trip {i}')
            for i in range(100)
        ]
        ...
        self.assertEqual(Trip.objects.filter(title__startswith='Bulk trip').count(), 100)
...
class MemoryUsageTests(TransactionTestCase):
    ...
    def test_queryset_iterator_memory_efficiency(self):
        ...
        Trip.objects.bulk_create([
            Trip(user=user, title=f'Iterator trip {i}')
            for i in range(1000)
        ])
...
class ScalabilityTests(TransactionTestCase):
    ...
    def test_pagination_performance(self):
        ...
        # Create many trips
        Trip.objects.bulk_create([
            Trip(user=user, title=f'Page trip {i}')
            for i in range(500)
        ])
    ...
    def test_search_performance(self):
        ...
        self.assertLess(
            search_time,
            0.5,
            f"Search took {search_time:.2f}s, should be under 0.5s"
        )
        ...
        def simulate_user_activity(user_index):
            try:
                ...
                if token_response.status_code == 200:
                    ...
                    # Get destinations
                    client.get('/api/destinations/', HTTP_AUTHORIZATION=f'Bearer {token}')
