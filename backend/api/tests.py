...
class TripModelTests(TestCase):
    ...
    def test_is_future_trip(self):
        ...
        future_trip = Trip.objects.create(
            user=self.user, title="Future", start_date=date.today() + timedelta(days=30)
        )
        self.assertTrue(future_trip.is_future_trip())

        # Past trip
        past_trip = Trip.objects.create(
            user=self.user, title="Past", start_date=date.today() - timedelta(days=30)
        )
...
class PlanningSessionModelTests(TestCase):
    ...
    def test_planning_session_creation(self):
        ...
        session = PlanningSession.objects.create(
            trip=self.trip, current_stage="destination"
        )
...
class PlanningSessionAPITests(APITestCase):
    ...
    def test_create_planning_session(self):
        ...
        data = {"trip": self.trip.id, "current_stage": "destination"}

        response = self.client.post(url, data, format="json")

        self.assertEqual(response.status_code, status.HTTP_201_CREATED)
        self.assertTrue(PlanningSession.objects.filter(trip=self.trip).exists())
    ...
    def test_advance_stage(self):
        ...
        response = self.client.post(
            f"/api/planning-sessions/{session.id}/advance_stage"
        )
...
class UserPreferencesAPITests(APITestCase):
    ...
    def setUp(self):
        ...
        self.preferences = UserPreferences.objects.create(
            user=self.user, preferences_text="Beach lover"
        )
        self.client.force_authenticate(user=self.user)
    ...
    def test_update_preferences(self):
        ...
        response = self.client.patch(
            f"/api/user-preferences/{self.preferences.id}", data, format="json"
        )
...
class SerializerValidationTests(TestCase):
    ...
    def test_user_serializer_creates_preferences(self):
        ...
        serializer = UserSerializer(
            data={"username": "newuser", "password": "pass123", "email": "new@test.com"}
        )
...
class PlanningSessionActionsTests(APITestCase):
    ...
    def test_advance_to_completion(self):
        ...
        session = PlanningSession.objects.create(
            trip=self.trip, current_stage="finalization"
        )
    ...
    def test_cannot_access_other_users_session(self):
        ...
        response = self.client.post(
            f"/api/planning-sessions/{other_session.id}/advance_stage"
        )
        self.assertEqual(response.status_code, status.HTTP_404_NOT_FOUND)
...
class TripValidationTests(APITestCase):
    ...
    def setUp(self):
        ...
        self.trip = Trip.objects.create(
            user=self.user, title="Test Trip"
        )  # ADDED THIS LINE
    ...
    def test_status_field_is_readonly(self):
        ...
        response = self.client.patch(
            f"/api/trips/{trip.id}", {"status": "booked"}, format="json"
        )
...
class PermissionTests(APITestCase):
    ...
    def test_cannot_update_other_users_trip(self):
        ...
        response = self.client.patch(
            f"/api/trips/{other_trip.id}", {"title": "Hacked"}, format="json"
        )
