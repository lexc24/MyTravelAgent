...
class MessageModelTests(TestCase):
    ...
    def test_message_creation_user_message(self):
        ...
        message = Message.objects.create(
            conversation=self.convo, is_user=True, content="hello Ai"
        )
    ...
    def test_message_creation_ai_message(self):
        ...
        message = Message.objects.create(
            conversation=self.convo, is_user=False, content="Hello! How can I help you?"
        )
    ...
    def test_message_ordering(self):
        ...
        msg1 = Message.objects.create(
            conversation=self.convo, is_user=True, content="First message"
        )
        msg2 = Message.objects.create(
            conversation=self.convo, is_user=False, content="Second message"
        )
        msg3 = Message.objects.create(
            conversation=self.convo, is_user=True, content="Third message"
        )
    ...
    def test_message_belongs_to_conversation(self):
        ...
        message = Message.objects.create(
            conversation=self.convo, is_user=True, content="Test message"
        )
...
class ConversationStateModelTests(TestCase):
    ...
    def test_progress_calculation_zero_questions(self):
        ...
        state = ConversationState.objects.create(
            conversation=self.conversation, total_questions=0
        )
    ...
    def test_progress_calculation_partial(self):
        ...
        state = ConversationState.objects.create(
            conversation=self.conversation, questions_asked=3, total_questions=5
        )
    ...
    def test_progress_calculation_complete(self):
        ...
        state = ConversationState.objects.create(
            conversation=self.conversation, questions_asked=5, total_questions=5
        )
    ...
    def test_is_complete_with_complete_stages(self):
        ...
        state = ConversationState.objects.create(
            conversation=self.conversation, current_stage="destinations_complete"
        )
    ...
    def test_is_complete_with_incomplete_stages(self):
        ...
        state = ConversationState.objects.create(
            conversation=self.conversation, current_stage="initial"
        )
...
class RecommendationsModelTests(TestCase):
    ...
    def test_recommendations_creation(self):
        ...
        recommendations = Recommendations.objects.create(
            conversation=self.conversation, locations=locations
        )
    ...
    def test_multiple_recommendations_per_conversation(self):
        ...
        recs1 = Recommendations.objects.create(
            ...
            locations=[
                {"name": "Paris", "country": "France", "description": "First set"}
            ],
        )
        ...
        recs2 = Recommendations.objects.create(
            conversation=self.conversation,
            locations=[
                {"name": "London", "country": "UK", "description": "Second set"}
            ],
        )
...
class ChatMessageAPITests(APITestCase):
    ...
    def test_chat_requires_trip_id(self):
        ...
        response = self.client.post("/destination_search/chat", {"message": "Hello"})
    ...
    def test_chat_requires_message(self):
        ...
        response = self.client.post(
            "/destination_search/chat", {"trip_id": self.trip.id}
        )
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
        self.assertIn("error", response.data)
    ...
    def test_chat_empty_message_rejected(self):
        ...
        response = self.client.post(
            "/destination_search/chat", {"trip_id": self.trip.id, "message": "   "}
        )
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
    ...
    def test_chat_validates_trip_ownership(self):
        ...
        response = self.client.post(
            ...
            {"trip_id": other_trip.id, "message": "test message"},
        )
    ...
    def test_chat_nonexistent_trip(self):
        ...
        response = self.client.post(
            "/destination_search/chat", {"trip_id": 99999, "message": "test message"}
        )
        self.assertEqual(response.status_code, status.HTTP_404_NOT_FOUND)
    ...
    def test_chat_initial_message_success(self, mock_workflow_manager):
        ...
        response = self.client.post(
            "/destination_search/chat",
            {"trip_id": self.trip.id, "message": "I want a beach vacation"},
        )
        ...
        self.assertEqual(
            Message.objects.filter(conversation__trip=self.trip).count(), 2
        )
        self.assertTrue(
            ConversationState.objects.filter(
                conversation__trip=self.trip, current_stage="asking_clarifications"
            ).exists()
        )
    ...
    def test_chat_updates_trip_status(self, mock_workflow_manager):
        ...
        response = self.client.post(
            "/destination_search/chat", {"trip_id": self.trip.id, "message": "Hello"}
        )
    ...
    def test_chat_handles_workflow_error(self, mock_workflow_manager):
        ...
        mock_workflow_manager.process_initial_message.side_effect = Exception(
            "Workflow error"
        )
        ...
        self.assertIn("error", response.data)
        self.assertNotIn(
            "Workflow error", response.data["error"]
...
class GetConversationAPITests(APITestCase):
    ...
    def test_get_existing_conversation_with_messages(self):
        ...
        conversation = TripConversation.objects.create(trip=self.trip)
        msg1 = Message.objects.create(
            conversation=conversation, is_user=True, content="Hello"
        )
        msg2 = Message.objects.create(
            conversation=conversation, is_user=False, content="Hi there!"
        )
    ...
    def test_get_conversation_with_state(self):
        ...
        self.assertEqual(
            response.data["state"]["current_stage"], "asking_clarifications"
        )
...
class ResetConversationAPITests(APITestCase):
    ...
    def test_reset_requires_authentication(self):
        ...
        self.client.force_authenticate(user=None)
        response = self.client.post(
            f"/destination_search/conversations/{self.trip.id}/reset"
        )
        self.assertEqual(response.status_code, status.HTTP_401_UNAUTHORIZED)
    ...
    def test_reset_conversation_success(self):
        ...
        self.assertEqual(
            Message.objects.filter(conversation__trip=self.trip).count(), 0
        )
    ...
    def test_cannot_reset_other_users_conversation(self):
        ...
        response = self.client.post(
            f"/destination_search/conversations/{other_trip.id}/reset"
        )
        self.assertEqual(response.status_code, status.HTTP_404_NOT_FOUND)
...
class HelperFunctionTests(TestCase):
    ...
    def test_handle_post_destination_message_commitment_detected(self):
        ...
        state = ConversationState.objects.create(
            conversation=conversation, current_stage="destinations_complete"
        )
        ...
        response = handle_post_destination_message(
            "Let's go with Paris!", conversation, state, trip
        )
    ...
    def test_handle_post_destination_message_no_commitment(self):
        ...
        response = handle_post_destination_message(
            "Tell me more about Paris", conversation, state, trip
        )
...
class SerializerTests(TestCase):
    ...
    def test_message_serializer(self):
        ...
        message = Message.objects.create(
            conversation=self.conversation, is_user=True, content="Test message"
        )
    ...
    def test_conversation_serializer_includes_related(self):
        ...
        Message.objects.create(
            conversation=self.conversation, is_user=True, content="Hello"
        )
...
def test_chat_blocks_sql_injection_attempts(self):
    ...
    response = self.client.post(
        "/destination_search/chat",
        {"trip_id": self.trip.id, "message": "'; DROP TABLE users; --"},
    )
    self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
...
class RateLimitingTests(APITestCase):
    ...
    def test_chat_rate_limiting_enforced(self):
        ...
        for i in range(10):
            response = self.client.post(
                "/destination_search/chat",
                {"trip_id": self.trip.id, "message": f"Test message {i}"},
            )
            self.assertEqual(response.status_code, status.HTTP_200_OK)
        ...
        response = self.client.post(
            "/destination_search/chat",
            {"trip_id": self.trip.id, "message": "This should be blocked"},
        )
...
def test_chat_sanitizes_html_input(self):
    ...
    with patch("destination_search.views.workflow_manager") as mock_wf:
        ...
        response = self.client.post(
            "/destination_search/chat",
            {"trip_id": self.trip.id, "message": "<script>alert('xss')</script>Hello"},
        )
        ...
        saved_message = Message.objects.filter(
            conversation__trip=self.trip, is_user=True
        ).last()
